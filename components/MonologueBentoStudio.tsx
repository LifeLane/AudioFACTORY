...
export const MonologueBentoStudio: React.FC<MonologueBentoStudioProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#F7F7F4] overflow-y-auto custom-scrollbar p-3 sm:p-5 md:p-6 space-y-4">
      ...
      {/* Error Notice */}
      {audioError && (
        <div className="p-3 rounded-xl border border-rose-200 bg-rose-50 text-rose-800 font-mono text-xs flex justify-between items-start z-10 shadow-2xs">
          <div className="break-all whitespace-pre-wrap flex-1 max-h-[160px] overflow-y-auto pr-2">
            {audioError}
          </div>
          <button onClick={onDismissError} className="font-bold text-rose-700 ml-4 hover:text-rose-900 flex-shrink-0">✕</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        {/* BENTO CARD 1: Script Editor (8 cols) */}
        <div className="lg:col-span-8 border border-zinc-200 bg-white rounded-xl shadow-2xs flex flex-col min-h-[360px] overflow-hidden">
          ...
          <div className="p-3.5 sm:px-5 sm:py-3.5 border-b border-zinc-100 bg-zinc-50/70 flex flex-wrap items-center justify-between gap-2 flex-shrink-0">
            <div className="flex items-center gap-2">
              <label className="text-xs font-mono uppercase font-bold text-zinc-900 tracking-wider flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5 text-zinc-500" /> Script
              </label>
            </div>
          ...
          <div className="flex-1 p-3.5 sm:p-5 flex flex-col min-h-0">
            <textarea
              ...
              placeholder="Type or paste your spoken script here..."
            />
            ...
            <div className="mt-3 pt-2.5 border-t border-zinc-100 flex flex-wrap items-center justify-between gap-2 text-xs font-mono text-zinc-500 flex-shrink-0">
              <div className="flex items-center gap-1.5 text-zinc-600 text-[11px]">
                <span>💡 Use ellipses (...) for realistic pauses.</span>
              </div>
              ...
              <div className="flex items-center gap-3 text-[11px]">
                ...
                <span>•</span>
                <span className="text-amber-700 font-bold">~{estimatedReadSeconds}s runtime</span>
              </div>
            </div>
          </div>
        ...
        {/* BENTO CARD 2: Voice Profile (4 cols) */}
        <div className="lg:col-span-4 border border-zinc-200 bg-white p-4 sm:p-5 rounded-xl shadow-2xs flex flex-col justify-between space-y-3">
      ...
      {/* BENTO CARD 3: Master Audio Transport & Waveform Station (Full Width, below the script) */}
      <div className="border border-zinc-200 bg-white p-4 sm:p-5 rounded-xl shadow-2xs flex flex-col justify-between space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title={isPlaying ? "Pause" : generatedAudio ? "Play" : "Synthesize"}
            >
              {isLoading ? (
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm sm:text-base font-bold text-zinc-900 uppercase">
                  {isPlaying ? "Playing Master" : generatedAudio ? "Audio Ready" : "Synthesizer"}
                </span>
              ...
              <p className="text-xs font-mono text-zinc-500 mt-0.5">
                {audioDuration > 0 
                  ...
                  : `Est. ~${estimatedReadSeconds}s runtime`}
              </p>
            </div>
          </div>
        ...
        <div className="pt-2">
          <div className="flex items-center justify-between text-[11px] font-mono text-zinc-500 mb-1.5">
            <span className="font-bold flex items-center gap-1">
              <Volume2 className="w-3.5 h-3.5 text-zinc-400" /> Audio Waveform
            </span>
